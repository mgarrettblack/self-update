...
| `update.go` | The orchestration layer, and the only file that calls `net.go`, `release.go` and `fs.go` together. Three concerns in sequence: **`Checker`** + `Decision` + `Check` (manifest and `.sig` fetch, the newer/platform/rollout decision, size caps); **`Guard`** + `Marker` + `StartupResult` + `MarkPending`/`CheckStartup`/`MarkHealthy` (attempt accounting); **`Poller`** + `UpdateResult` + `ErrRestartRequired` + `Run`/`Update`/`apply`/`Startup`/`MarkHealthy`/`target`. | [../update-cycle/check.md](../update-cycle/check.md), [../update-cycle/rollback.md](../update-cycle/rollback.md), [../update-cycle/lifecycle-ordering.md](../update-cycle/lifecycle-ordering.md) |
...
(the state dir must be writable even when the install directory is not). `Update`
